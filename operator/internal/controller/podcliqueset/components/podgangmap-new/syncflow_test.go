	...
	tests := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, pgm *grovecorev1alpha1.PodGangMap) {
				...
				assert.Nil(t, pgm.Status.UpdateProgress, "Status is not written on the empty-PGM path")
			},
		},
		{
...
func TestReconcileSteadyStateEntries(t *testing.T) {
	...
	stalePGM := testutils.NewPodGangMapBuilder(testPCSName, testNamespace, testPCSUID, 0).
		WithEntries(testutils.NewPodGangEntryBuilder("pg-0", testHash, "e0").WithEpochAnchor(true).Build()).
		WithUpdateProgress(&grovecorev1alpha1.PodGangMapUpdateProgress{}).Build()
	cl := newFakeClientTrackingStatus([]client.Object{pcs, pclq, pcsg, stalePGM}, stalePGM)
	r := newResource(cl)
	snap := snapshotFor(pcs,
		map[int][]grovecorev1alpha1.PodClique{0: {*pclq}},
		map[int][]grovecorev1alpha1.PodCliqueScalingGroup{0: {*pcsg}},
		map[int]grovecorev1alpha1.PodGangMap{0: *stalePGM})

	require.NoError(t, r.reconcileSteadyStateEntries(context.Background(), snap, 0))

	pgm := getPGM(t, r.client, 0)
	assert.NotEmpty(t, pgm.Spec.Entries)
	assert.Nil(t, pgm.Status.UpdateProgress, "stale UpdateProgress is cleared")
}
...
func TestReconcileCoherentUpdateEntries(t *testing.T) {
	...
	cl := newFakeClientTrackingStatus([]client.Object{pcs, pclq, pcsg, pgm}, pgm)
	r := newResource(cl)
...
func TestPatchPodGangMapStatus(t *testing.T) {
	tests := []struct {
		name     string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			pcs := pcsWithCoherentUpdateStrategy(testHash).Build()
			pgm := pgmWithEpochEntry(0, "e0")
			r := newResource(newFakeClientTrackingStatus([]client.Object{pcs, pgm}, pgm))

			require.NoError(t, r.patchPodGangMapStatus(context.Background(), pcs, pgm.Name, tc.progress))

			got := getPGM(t, r.client, 0).Status.UpdateProgress
			if tc.wantNil {
				assert.Nil(t, got)
			} else {
				assert.NotNil(t, got)
...
func TestRunSyncFlow(t *testing.T) {
	tests := []struct {
		...
		{
			name: "steady-state replica rebuilds entries and clears stale progress",
			pcs:  pcsWithCoherentUpdateStrategy(testHash).Build(),
			objects: []client.Object{
				standalonePCLQWithMapping("frontend", 0, map[string]int32{"pg-0": 2}),
			...
			statusObjs: []client.Object{
				testutils.NewPodGangMapBuilder(testPCSName, testNamespace, testPCSUID, 0).Build(),
			},
			assert: func(t *testing.T, cl client.Client) {
				pgm := getPGM(t, cl, 0)
				assert.NotEmpty(t, pgm.Spec.Entries)
				assert.Nil(t, pgm.Status.UpdateProgress)
			},
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cl := testutils.NewTestClientBuilder().WithObjects(objs...).WithStatusSubresource(tc.statusObjs...).Build()
			r := newResource(cl)
...
func standalonePCLQWithMapping(clique string, pcsReplicaIndex int32, mapping map[string]int32) *grovecorev1alpha1.PodClique {
	...
	return testutils.NewPodCliqueBuilder(testPCSName, testPCSUID, clique, testNamespace, pcsReplicaIndex).
		WithReplicas(replicas).WithPodGangMapping(mapping).Build()
}

// prefillPCSGWithMapping builds the prefill PodCliqueScalingGroup for the given replica with a PodGangMapping.
...
// newFakeClientTrackingStatus builds a fake client seeded with objs whose status subresource is
// tracked for statusObjs, so Status patches are observable.
func newFakeClientTrackingStatus(objs []client.Object, statusObjs ...client.Object) client.Client {
	return testutils.NewTestClientBuilder().WithObjects(objs...).WithStatusSubresource(statusObjs...).Build()
}
