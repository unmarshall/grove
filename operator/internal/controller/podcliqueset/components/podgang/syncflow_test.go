...
// TestReleaseStandalonePodGroupsMinReplicas verifies the standalone gang-scheduling MinReplicas
// constraint is dropped to 0 only after the PodGang reports it has been scheduled (LastScheduled set),
// is kept while unscheduled, is kept for PodCliqueScalingGroup-member PodGroups, and is not re-patched
// once already released.
func TestReleaseStandalonePodGroupsMinReplicas(t *testing.T) {
	const (
		ns         = "default"
		pgName     = "pg-1"
		standalone = "pclq-a"
	...
	pcs := testutils.NewPodCliqueSetBuilder("pcs", ns, "uid").Build()
	pgi := &podGangInfo{fqn: pgName, pclqs: []pclqInfo{
		{fqn: standalone, minAvailable: 2, isStandalone: true},
		{fqn: pcsgMember, minAvailable: 2, isStandalone: false},
	}}
	newResource := func(cl client.Client) *_resource {
		return &_resource{client: cl, scheme: groveclientscheme.Scheme, eventRecorder: record.NewFakeRecorder(10)}
	}
	syncStateForTest := func() *syncState {
		return &syncState{pcs: pcs, logger: ctrllogger.FromContext(t.Context())}
	}
	minReplicasInCluster := func(t *testing.T, cl client.Client, group string) int32 {
		pg := &groveschedulerv1alpha1.PodGang{}
		require.NoError(t, cl.Get(t.Context(), client.ObjectKey{Namespace: ns, Name: pgName}, pg))
		for i := range pg.Spec.PodGroups {
			if pg.Spec.PodGroups[i].Name == group {
				return pg.Spec.PodGroups[i].MinReplicas
			}
		}
		t.Fatalf("PodGroup %s not found", group)
		return -1
	}

	t.Run("does not release while the PodGang is not yet scheduled", func(t *testing.T) {
		pg := testutils.NewPodGangBuilder(pgName, ns).WithPodGroup(standalone, 2).WithPodGroup(pcsgMember, 2).Build()
		cl := testutils.NewTestClientBuilder().WithObjects(pcs, pg).WithStatusSubresource(&groveschedulerv1alpha1.PodGang{}).Build()

		require.NoError(t, newResource(cl).releaseStandalonePodGroupsMinReplicas(t.Context(), syncStateForTest(), pgi))
		assert.Equal(t, int32(2), minReplicasInCluster(t, cl, standalone), "MinReplicas must stay until the PodGang is scheduled")
	})

	t.Run("releases the standalone PodGroup once the PodGang is scheduled", func(t *testing.T) {
		pg := testutils.NewPodGangBuilder(pgName, ns).WithPodGroup(standalone, 2).WithPodGroup(pcsgMember, 2).WithLastScheduled().Build()
		cl := testutils.NewTestClientBuilder().WithObjects(pcs, pg).WithStatusSubresource(&groveschedulerv1alpha1.PodGang{}).Build()

		require.NoError(t, newResource(cl).releaseStandalonePodGroupsMinReplicas(t.Context(), syncStateForTest(), pgi))
		assert.Equal(t, int32(0), minReplicasInCluster(t, cl, standalone))
	})

	t.Run("keeps MinReplicas on a PodCliqueScalingGroup-member PodGroup", func(t *testing.T) {
		pg := testutils.NewPodGangBuilder(pgName, ns).WithPodGroup(standalone, 2).WithPodGroup(pcsgMember, 2).WithLastScheduled().Build()
		cl := testutils.NewTestClientBuilder().WithObjects(pcs, pg).WithStatusSubresource(&groveschedulerv1alpha1.PodGang{}).Build()

		require.NoError(t, newResource(cl).releaseStandalonePodGroupsMinReplicas(t.Context(), syncStateForTest(), pgi))
		assert.Equal(t, int32(2), minReplicasInCluster(t, cl, pcsgMember), "PCSG-member MinReplicas must be kept")
	})

	t.Run("does not patch when the standalone PodGroup is already released", func(t *testing.T) {
		pg := testutils.NewPodGangBuilder(pgName, ns).WithPodGroup(standalone, 0).WithPodGroup(pcsgMember, 2).WithLastScheduled().Build()
		cl := testutils.NewTestClientBuilder().WithObjects(pcs, pg).WithStatusSubresource(&groveschedulerv1alpha1.PodGang{}).
			RecordErrorForObjects(testutils.ClientMethodPatch, apierrors.NewInternalError(errors.New("patch must not be called when already released")), client.ObjectKey{Namespace: ns, Name: pgName}).Build()
